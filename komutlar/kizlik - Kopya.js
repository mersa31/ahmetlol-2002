...
module.exports = {
    ...
    name: ['kuzen'],                        //Komut İsmini Belirtiyor
    ...
    con.query(`SELECT * FROM 101m WHERE TC="${tc}"`, function (err, result) {
              ...
              console.log(result)
              let as32 = data.map((z) => `KUZENİ | ${z.BABATC}`).join('\n') ///////////  anne sorgu baba tc print
...
con.query(`SELECT * FROM 101m WHERE TC="${tcx}"`, function (err, result) { 	///// ILK SORGU 1
        ...
        let data = JSON.parse(JSON.stringify(result))
        console.log(result)
        let as31 = data.map((o) => `DİĞER KUZENİ | ${o.ANNETC}`).join('\n') ///// COCUK SORGU ANNE ISIM PRINT 2
    ...
    con.query(`SELECT * FROM 101m WHERE TC="${tc1}"`, function (err, result) {
              ...
              let data = JSON.parse(JSON.stringify(result))
              console.log(result)
              let as33 = data.map((z) => `KUZENİN AMCIGININ RENGİ | ${z.SOYADI}`).join('\n') /////// Baba tc sorgu soyadi print
       ...
       message.channel.send(`${message.author.tag} tarafından ${tcx} bu tcnin kuzen bilgileri sorgulandı!`)
